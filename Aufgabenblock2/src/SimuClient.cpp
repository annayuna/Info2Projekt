...
void AfxMessageBox(CString msg) {
	std::cout << "### SimuClient MESSAGE: " << msg << std::endl;
}
...
SIMUCLIENT_API bool bInitialisiereGrafik(int sizeX, int sizeY, const char* address, const char* port) {
   ...
   string programName = "java -jar src/SimuServer.jar";
   ...
   if (!ClientSocket->Create()) {
      AfxMessageBox(CString("TCP/IP-Verbindung nicht moeglich!"));
      return false;
   }
   ...
   if (iCount < 0) {
         AfxMessageBox(CString("Connect nicht moeglich! (Firewall ?)"));
         AfxMessageBox(CString("Grafikausgaben werden unterdrueckt."));
         return false;
   }
   stringstream msg;
...
SIMUCLIENT_API void vBeendeGrafik() {
   ...
   if (initialized)
   {
      AfxMessageBox(CString("Simulation beendet."));
      stringstream msg;
      msg << "close#";
      ClientSocket->Send(msg);
      ClientSocket->Close();
